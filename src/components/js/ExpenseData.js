...
const ExpenseData = () => {
  ...
  useEffect(() => {
    const fetchExpenseData = async () => {
      try {
        const response = await fetch("http://localhost:6999/expensedata", {
          method: "GET",
          headers: {
  ...
  const handleSave = async (expenseId, userId) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:6999/expensedata/${expenseId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDelete = (expenseId, userId) => {
    ...
    if (confirmation) {
      fetch(`http://localhost:6999/expensedata/${expenseId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authenticate: `Bearer ${token}`,
            ...
            user.expenses.map((expense, expenseIndex) => (
              <tr key={expense._id}>
                {editIndex === expenseIndex ? (
                  <>
                    <td>
                      {/* Make select read-only when editing */}
                      <select
                        name="month"
