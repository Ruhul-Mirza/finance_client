...
const ExpenseData = () => {
  ...
  useEffect(() => {
    const fetchExpenseData = async () => {
      try {
        ...
        console.log("Fetched data:", result); // Verify month data in the API response
  ...
  const handleEdit = (index, expense) => {
    setEditIndex(index);
    setEditedExpense({ ...expense }); // Ensure the month field is included here
    setSelectHome(expense.home);
    console.log("Editing expense:", expense); // Log to verify the month field
  };
  ...
  const handleSave = async (expenseId, userId) => {
    const total =
      (parseFloat(editedExpense.home || 0) || 0) +
      (parseFloat(editedExpense.rentAmount || 0) || 0) +
      (parseFloat(editedExpense.foodAmount || 0) || 0) +
      (parseFloat(editedExpense.entertainmentAmount || 0) || 0) +
      (parseFloat(editedExpense.utilitiesAmount || 0) || 0) +
      (parseFloat(editedExpense.personalAmount || 0) || 0) +
      (parseFloat(editedExpense.othersAmount || 0) || 0);
    ...
    <tbody>
      {expenseData.map((user, userIndex) => 
        user.expenses.map((expense, expenseIndex) => {
          ...
          console.log("Expense being rendered:", expense); // Check if 'month' and other values are correct
              ...
              {editIndex === expenseIndex ? (
                <>
                  <td>
                    <select
                      name="month"
                      value={editedExpense.month || ""} // Bind to the month value in editedExpense
